...
class FileDatasetsIter(IterableDataset):
    ...
    def populate_buffer(self, file_list):
            ...
            for game in file:
                ...
                try:
                    ...
                    if len(kyoku_rewards) < required_length:
                        skipped_games += 1
                        logging.debug(f"Skipping game due to insufficient kyoku_rewards. "
                                    f"Required: {required_length}, Got: {len(kyoku_rewards)}, "
                                    f"Max kyoku index: {max_kyoku_idx}")
                        continue
                    ...
                    if len(player_ranks) <= max_kyoku_idx + 1:
                        skipped_games += 1
                        logging.debug(f"Skipping game due to insufficient player_ranks. "
                                    f"Required: {max_kyoku_idx + 2}, Got: {len(player_ranks)}")
                        continue
                    ...
                    for i in range(game_size):
                        ...
                        if kyoku_idx >= len(kyoku_rewards):
                            logging.warning(f"Kyoku index {kyoku_idx} out of bounds for kyoku_rewards length {len(kyoku_rewards)}")
                            break
                        if kyoku_idx + 1 >= len(player_ranks):
                            logging.warning(f"Player rank index {kyoku_idx + 1} out of bounds for player_ranks length {len(player_ranks)}")
                            break
        ...
        if skipped_games > 0:
            logging.info(f"Skipped {skipped_games}/{total_games} games due to data inconsistencies")
